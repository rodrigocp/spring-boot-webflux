...
abstract class AbstractHandler<R : Record, D> (protected val repository : Repository<R>, protected val mapper : Mapper<R, D>) : Handler {
	...
	override suspend fun fetch(request: ServerRequest): ServerResponse {
		return ServerResponse.ok().contentType(APPLICATION_JSON).bodyValueAndAwait(repository.find().map { mapper.toDTO(it) })
	}
